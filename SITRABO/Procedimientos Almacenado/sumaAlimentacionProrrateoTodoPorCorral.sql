CREATE DEFINER=`root`@`localhost` PROCEDURE `sumaAlimentacionProrrateoTodoPorCorral`(
	in fecD date,
    in fecH date,
    in idC int(11)
)
BEGIN
	...
	WHERE Fecha >= fecD AND Fecha <= fecH AND Detalle_Tropa.CORRALES_idCorrales = idC
	ORDER BY
		Fecha ASC;
END