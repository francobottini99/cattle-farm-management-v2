CREATE DEFINER=`root`@`localhost` PROCEDURE `consVentasSinComercializarUs`(
	in idP int(11),
    in Fec date,
	in idUs int(11)
)
BEGIN
	...
	WHERE
		Ventas.Comprador = "-" AND Ventas.USUARIOS_idUsuario = idUS AND Productor.idProductor = idP AND Ventas.Fecha = Fec
