...
Public Class frmManejoAlimentacion
    ...
    Private Sub Cancelar()
        ...
        frmGuardarAlimentacion.lblGuardado.Visible = False
    ...
    Private Sub pbxGuardar_Click(sender As Object, e As EventArgs) Handles pbxGuardar.Click
        ...
        Try
            Dim CostG, CostF, CostC, CostO, PGr, PCo, PFb, POt, Peso, ConsumoT, TotalConsT, PorCons As Double
                        ...
                        For i As Integer = 0 To dgvCorralesSel.Rows.Count - 1
                            ...
                            For j As Integer = rows To 0 Step -1
                                If dgvRacion.Rows(j).Cells.Item(1).Value = dgvCorralesSel.Rows(i).Cells.Item(2).Value Then
                                    CostG = dgvCorralesSel.Rows(i).Cells.Item(4).Value * CDbl(dgvRacion.Rows(j).Cells.Item(6).Value)
                                    CostC = dgvCorralesSel.Rows(i).Cells.Item(5).Value * CDbl(dgvRacion.Rows(j).Cells.Item(7).Value)
                                    CostF = dgvCorralesSel.Rows(i).Cells.Item(6).Value * CDbl(dgvRacion.Rows(j).Cells.Item(8).Value)
                                    CostO = dgvCorralesSel.Rows(i).Cells.Item(7).Value * CDbl(dgvRacion.Rows(j).Cells.Item(9).Value)
                                End If
                            Next

                            PGr = (dgvCorralesSel.Rows(i).Cells.Item(4).Value / dgvCorralesSel.Rows(i).Cells.Item(3).Value) * 100
                            PCo = (dgvCorralesSel.Rows(i).Cells.Item(5).Value / dgvCorralesSel.Rows(i).Cells.Item(3).Value) * 100
                            PFb = (dgvCorralesSel.Rows(i).Cells.Item(6).Value / dgvCorralesSel.Rows(i).Cells.Item(3).Value) * 100
                            POt = (dgvCorralesSel.Rows(i).Cells.Item(7).Value / dgvCorralesSel.Rows(i).Cells.Item(3).Value) * 100

                            G.Grabar("Alimentacion", "NoImagen", "feedlot.insAlimentacion", Format(CDate(dgvCorralesSel.Rows(i).Cells.Item(0).Value), "yyyy/MM/dd"), CDbl(dgvCorralesSel.Rows(i).Cells.Item(3).Value), _
                                     "-", CDbl(dgvCorralesSel.Rows(i).Cells.Item(4).Value), CDbl(dgvCorralesSel.Rows(i).Cells.Item(5).Value), CDbl(dgvCorralesSel.Rows(i).Cells.Item(6).Value), _
                                     CDbl(dgvCorralesSel.Rows(i).Cells.Item(7).Value), CostG, CostC, CostF, CostO, (CostC + CostG + CostF + CostO), _
                                     Format(PGr, "#,##0.00"), Format(PCo, "#,##0.00"), Format(PFb, "#,##0.00"), Format(POt, "#,##0.00"), "-", _
                                     CInt(dgvCorralesSel.Rows(i).Cells.Item(8).Value), CInt(dgvCorralesSel.Rows(i).Cells.Item(9).Value), CInt(frmLogin.txtID.Text))
                            ...
                            _Dt2 = _Ver2.verStkCorralFec(CInt(frmLogin.txtID.Text), dgvCorralesSel.Rows(i).Cells.Item(8).Value, Fecha)
                            ...
                            For Each _Dr As DataRow In _Dt2.Rows
                                ...
                                If _Dt3.Rows.Count > 0 And DateDiff(DateInterval.Day, _Dt3.Rows(0).Item(1), dgvCorralesSel.Rows(i).Cells.Item(0).Value) > 0 Then
                            ...
                            For j As Integer = 0 To x - 1
                                ...
                                G.Grabar("alimentacion_prorrateo", "NoImagen", "feedlot.insAlimentacionProrrateo", Format(CDate(dgvCorralesSel.Rows(i).Cells.Item(0).Value), "yyyy/MM/dd"), CDbl(dgvCorralesSel.Rows(i).Cells.Item(3).Value) * PorCons,
                                       CDbl(dgvCorralesSel.Rows(i).Cells.Item(4).Value) * PorCons, CDbl(dgvCorralesSel.Rows(i).Cells.Item(5).Value) * PorCons, CDbl(dgvCorralesSel.Rows(i).Cells.Item(6).Value) * PorCons,
                                        CDbl(dgvCorralesSel.Rows(i).Cells.Item(7).Value) * PorCons, CostG * PorCons, CostC * PorCons, CostF * PorCons, CostO * PorCons, (CostC + CostG + CostF + CostO) * PorCons,
                                        Format(PGr, "#,##0.00"), Format(PCo, "#,##0.00"), Format(PFb, "#,##0.00"), Format(POt, "#,##0.00"), CInt(dgvCorralesSel.Rows(i).Cells.Item(9).Value), CInt(Consumo(j, 1)),
                                    ...
                                    If resP = Windows.Forms.DialogResult.OK Then
                                        ...
                                        For j As Integer = rows To 0 Step -1
                                            If dgvRacion.Rows(j).Cells.Item(0).Value = txtIDRac.Text Then
                                                CostG = txtKGGrano.Text * CDbl(dgvRacion.Rows(j).Cells.Item(6).Value)
                                                CostC = txtKGConcentrado.Text * CDbl(dgvRacion.Rows(j).Cells.Item(7).Value)
                                                CostF = txtKGFibra.Text * CDbl(dgvRacion.Rows(j).Cells.Item(8).Value)
                                                CostO = txtKGOtros.Text * CDbl(dgvRacion.Rows(j).Cells.Item(9).Value)
                                            End If
                                        Next
                                        ...
                                        G.Grabar("Alimentacion", "NoImagen", "feedlot.modAlimentacion", txtIDModAlim.Text, Format(CDate(lblFecha.Text), "yyyy/MM/dd"), CDbl(txtKGTotales.Text), _
                                                 "-", CDbl(txtKGGrano.Text), CDbl(txtKGConcentrado.Text), CDbl(txtKGFibra.Text), _
                                                 CDbl(txtKGOtros.Text), CostG, CostC, CostF, CostO, (CostC + CostG + CostF + CostO), _
                                                 Format(PGr, "#,##0.00"), Format(PCo, "#,##0.00"), Format(PFb, "#,##0.00"), Format(POt, "#,##0.00"), "-", _
                                        ...
                                        For j As Integer = 0 To x - 1
                                            ...
                                            G.Grabar("alimentacion_prorrateo", "NoImagen", "feedlot.insAlimentacionProrrateo", Format(CDate(lblFecha.Text), "yyyy/MM/dd"), CDbl(txtKGTotales.Text) * PorCons, _
                                                CDbl(txtKGGrano.Text) * PorCons, CDbl(txtKGConcentrado.Text) * PorCons, CDbl(txtKGFibra.Text) * PorCons, _
                                                CDbl(txtKGOtros.Text) * PorCons, CostG * PorCons, CostC * PorCons, CostF * PorCons, CostO * PorCons, (CostC + CostG + CostF + CostO) * PorCons, _
                                                Format(PGr, "#,##0.00"), Format(PCo, "#,##0.00"), Format(PFb, "#,##0.00"), Format(POt, "#,##0.00"), CInt(txtIDRac.Text), CInt(Consumo(j, 1)), _
    ...
    Private Sub dgvRacion_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvRacion.CellClick
        txtInformacion.Text = "--------------------- RACION -----------------------" + vbNewLine + _
        vbNewLine + "Grano       : " + dgvRacion.CurrentRow.Cells.Item(2).Value + vbNewLine + "Concent.  : " + _
        dgvRacion.CurrentRow.Cells.Item(3).Value + vbNewLine + "Fibra        : " + dgvRacion.CurrentRow.Cells.Item(4).Value + _
        vbNewLine + "Otros        : " + dgvRacion.CurrentRow.Cells.Item(5).Value
    ...
    Private Sub dgvCorralesDisp_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvCorralesDisp.CellClick
        If dgvRacion.CurrentRow IsNot Nothing Then
            ...
            txtInformacion.Text = "--------------------- CORRAL ----------------------" + vbNewLine + vbNewLine + _
            ...
            Else
                ...
                Else
                    ...
                    Else
                        ...
                        dgvCorralesSel.Columns(5).Name = "KG Concentrado"
                        dgvCorralesSel.Columns(6).Name = "KG Fibra"
                        dgvCorralesSel.Columns(7).Name = "KG Otros"
                        dgvCorralesSel.Columns(8).Name = "IDCrr"
                        dgvCorralesSel.Columns(9).Name = "IDRac"

                        dgvCorralesSel.Columns(8).Visible = False
                        ...
                        If resP = Windows.Forms.DialogResult.OK Then
                            dgvCorralesSel.Rows.Add(lblFecha.Text, dgvCorralesDisp.CurrentRow.Cells.Item(1).Value, dgvRacion.CurrentRow.Cells.Item(1).Value, txtKGTotales.Text, txtKGGrano.Text, txtKGConcentrado.Text, txtKGFibra.Text, txtKGOtros.Text, dgvCorralesDisp.CurrentRow.Cells.Item(0).Value, dgvRacion.CurrentRow.Cells.Item(0).Value)
    ...
    Private Sub txtKGTotales_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtKGTotales.KeyPress
        ...
        ElseIf Asc(e.KeyChar) = Keys.Enter Then
            ...
            ElseIf txtKGGrano.Enabled = False Then
                ...
                If txtKGOtros.Enabled = False And txtKGFibra.Enabled Then
            ...
            Else
                ...
                If txtKGOtros.Enabled = False And txtKGFibra.Enabled = False And txtKGConcentrado.Enabled = False Then
            ...
            lblDifPorc.Text = Format(lblTotPorc.Text - lblGrPorc.Text - lblConcPorc.Text - lblFibPorc.Text - lblOtrPorc.Text, "#,##0.00")
        End If
    End Sub
    ...
    Private Sub txtKGGrano_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtKGGrano.KeyPress
        ...
        ElseIf Asc(e.KeyChar) = Keys.Enter Then
            ...
            If txtKGConcentrado.Enabled = False And txtKGFibra.Enabled = False And txtKGOtros.Enabled = False Then
                If txtOrigen.Text = "Nuevo" Then
                    pbxAgregar.Select()
                Else
                    pbxGuardar.Select()
                End If
            ElseIf txtKGConcentrado.Enabled = False And txtKGFibra.Enabled = False Then
                txtKGOtros.SelectionLength = Len(txtKGOtros.Text)
                txtKGOtros.Select()
            ElseIf txtKGConcentrado.Enabled = False Then
                txtKGFibra.SelectionLength = Len(txtKGFibra.Text)
                txtKGFibra.Select()
            Else
                txtKGConcentrado.SelectionLength = Len(txtKGConcentrado.Text)
    ...
    Private Sub txtKGDif_Validated(sender As Object, e As EventArgs) Handles txtKGTotales.Validated, txtKGGrano.Validated, txtKGConcentrado.Validated, txtKGFibra.Validated, txtKGOtros.Validated
        ...
        txtKGDif.Text = txtKGTotales.Text - txtKGGrano.Text - txtKGConcentrado.Text - txtKGFibra.Text - txtKGOtros.Text
        lblDifPorc.Text = Format(lblTotPorc.Text - lblGrPorc.Text - lblConcPorc.Text - lblFibPorc.Text - lblOtrPorc.Text, "#,##0.00")
    End Sub
    ...
    Private Sub dgvCorralesSel_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvCorralesSel.CellClick
        If dgvCorralesDisp.CurrentRow IsNot Nothing Then
            If dgvCorralesSel.CurrentRow.Cells.Item(2).Value = dgvRacion.CurrentRow.Cells.Item(1).Value Then
                ...
                txtKGConcentrado.Text = dgvCorralesSel.CurrentRow.Cells.Item(5).Value
                txtKGFibra.Text = dgvCorralesSel.CurrentRow.Cells.Item(6).Value
                txtKGOtros.Text = dgvCorralesSel.CurrentRow.Cells.Item(7).Value
            End If
        End If
        ...
        txtInformacion.Text = "-------- DETALLE ALIMENTACION -------" + _
        vbNewLine + _
        vbNewLine + "Grano        : " + CStr(dgvCorralesSel.CurrentRow.Cells.Item(4).Value) + " KG  -->  " + CStr(Format(CostG, "$ #,##0")) + _
        vbNewLine + "Concent.   : " + CStr(dgvCorralesSel.CurrentRow.Cells.Item(5).Value) + " KG  -->  " + CStr(Format(CostC, "$ #,##0")) + _
        vbNewLine + "Fibra         : " + CStr(dgvCorralesSel.CurrentRow.Cells.Item(6).Value) + " KG  -->  " + CStr(Format(CostF, "$ #,##0")) + _
        vbNewLine + "Otros         : " + CStr(dgvCorralesSel.CurrentRow.Cells.Item(7).Value) + " KG  -->  " + CStr(Format(CostO, "$ #,##0")) + _
    ...
    Private Sub dgvAlimentaciones_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvAlimentaciones.CellClick
        txtInformacionMod.Text = "------- DETALLE ALIMENTACION ---------" + _
            vbNewLine + _
            vbNewLine + "Racion: " + CStr(dgvAlimentaciones.CurrentRow.Cells.Item(19).Value) + _
            vbNewLine + _
            vbNewLine + "Grano        : " + CStr(dgvAlimentaciones.CurrentRow.Cells.Item(3).Value) + " KG  -->  " + CStr(Format(dgvAlimentaciones.CurrentRow.Cells.Item(11).Value, "$ #,##0")) + _
            vbNewLine + "Concent.   : " + CStr(dgvAlimentaciones.CurrentRow.Cells.Item(4).Value) + " KG  -->  " + CStr(Format(dgvAlimentaciones.CurrentRow.Cells.Item(12).Value, "$ #,##0")) + _
            vbNewLine + "Fibra         : " + CStr(dgvAlimentaciones.CurrentRow.Cells.Item(5).Value) + " KG  -->  " + CStr(Format(dgvAlimentaciones.CurrentRow.Cells.Item(13).Value, "$ #,##0")) + _
            vbNewLine + "Otros         : " + CStr(dgvAlimentaciones.CurrentRow.Cells.Item(6).Value) + " KG  -->  " + CStr(Format(dgvAlimentaciones.CurrentRow.Cells.Item(14).Value, "$ #,##0")) + _
